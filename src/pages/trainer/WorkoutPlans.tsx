...
import { Plus, Dumbbell, Trash2, ChevronRight, RefreshCw } from 'lucide-react'
...
export default function TrainerWorkoutPlans() {
  ...
  const [form, setForm] = useState({ name: '', description: '', repeatCycle: true })
  ...
  const createPlan = useMutation({
    ...
    onSuccess: (id) => {
      ...
      setModal(false)
      setForm({ name: '', description: '', repeatCycle: true })
      navigate(`/trainer/workout-plans/${id}`)
    },
  })
  ...
  const deletePlan = useMutation({
    mutationFn: async (id: string) => {
      ...
      await Promise.all(usersSnap.docs.flatMap((d) => {
        ...
        if (data.assignedWorkoutPlanId === id) updates.assignedWorkoutPlanId = null
        if (data.activeWorkoutPlanId === id) updates.activeWorkoutPlanId = null
        return Object.keys(updates).length ? [updateDoc(doc(db, 'users', d.id), updates)] : []
      }))
      const assignSnap = await getDocs(query(collection(db, 'assignments'), where('workoutPlanId', '==', id)))
      await Promise.all(assignSnap.docs.map((d) => updateDoc(doc(db, 'assignments', d.id), { workoutPlanId: null })))
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['trainer-workout-plans'] }),
  })
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#16213E]">Workout Plans</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-lg font-medium text-[#16213E] mb-2">No workout plans yet</h3>
          <p className="text-[#9E998F] text-sm mb-6">Create structured workout plans for your clients</p>
          <Button onClick={() => setModal(true)}>
            <Plus size={16} />
            Create First Plan
          </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {plans.map((plan) => (
            <Card key={plan.id} className="group cursor-pointer" onClick={() => navigate(`/trainer/workout-plans/${plan.id}`)}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-[#16213E] flex items-center justify-center flex-shrink-0">
                    <Dumbbell size={18} className="text-[#C9A84C]" />
                  </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Create Workout Plan">
